...
Status DescriptorTbl::CreateHdfsTblDescriptor(const TDescriptorTable& thrift_tbl,
    TableId tbl_id, ObjectPool* pool, MemTracker* mem_tracker,
    HdfsTableDescriptor** desc) {
  *desc = nullptr;
  for (const TTableDescriptor& tdesc: thrift_tbl.tableDescriptors) {
    if (tdesc.id == tbl_id) {
      DCHECK(tdesc.__isset.hdfsTable);
      RETURN_IF_ERROR(CreateTblDescriptorInternal(tdesc, pool, mem_tracker,
          reinterpret_cast<TableDescriptor**>(desc)));
      return Status::OK();
    }
  }
  return Status::OK();
...
Status DescriptorTbl::Create(ObjectPool* pool, const TDescriptorTable& thrift_tbl,
  ...
  *tbl = pool->Add(new DescriptorTbl());
  ...
  for (const TTableDescriptor& tdesc: thrift_tbl.tableDescriptors) {
    ...
    RETURN_IF_ERROR(CreateTblDescriptorInternal(tdesc, pool, mem_tracker, &desc));
